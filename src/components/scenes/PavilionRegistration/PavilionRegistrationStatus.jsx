...
const PavilionRegistrationStatus = ({ firebase }) => {
  ...
  const onSubmit = async () => {
    ...
    try {
      await firebase.updateUser(firebase.getCurrentUserId(), { dateLaunch: bbTakePlaced })
      setLoadSending(false)
      setAllowedToSubmit(false)
      addToast('Thank you! the information is saved successfully.', { appearance: 'success' })
    } catch (error) {
      setLoadSending(false)
  ...
  const renderCheckDateBox = () => (
    <>
      <p>In which period(s) of BB2020 will your Pavilion take place? select all that apply</p>
      <CheckBox
      ...
      <Button disabled={!allowedToSubmit} onClick={onSubmit}>submit</Button>
    </>
  )
        ...
        user.registrationStatus === RegistrationStatus.FINISHED_ADVANCE
          && (
            <div className="pavilion-registration-status__waiting-for-approval">
              <p className="pavilion-registration-status__cautious-text">May I have your attention, please ?</p>
              <div className="pavilion-registration-status__date-box">
                {
                  loadSending
                    ? <div style={{ position: 'relative', left: 'calc(50% - 17.5px)' }}><ClipLoader/></div>
                    : renderCheckDateBox()
                }
              </div>
