...
class SignInForm extends Component {
  ...
  onSubmit = async (event) => {
    ...
    try {
      const socialAuthUser = await this.props.firebase.doSignInWithEmailAndPassword(
        email,
        password,
      )
      const userSnapshot = await this.props.firebase.getUser(
    ...
    return (
      <>
        <form onSubmit={this.onSubmit}>
          ...
          {error && (
            <p style={{ color: '#FC0000' }}>{error.message}</p>
          )}
        </form>
      </>
    )
